...
func _physics_process(delta: float) -> void:
	...
	if abs(delta_h) > HORIZONTAL_DEAD_ZONE:
		global_position.x = move_toward(global_position.x, player.global_position.x, follow_speed * delta)
	
	# Follow vertically
	if player.is_on_floor():
		last_platform_y = player.global_position.y
		global_position.y = lerp(global_position.y, last_platform_y - VERTICAL_OFFSET, 0.1)
	elif player.velocity.y > 0:
		global_position.y = max(global_position.y, player.global_position.y - VERTICAL_OFFSET)
	else:
		global_position.y = lerp(global_position.y, last_platform_y - VERTICAL_OFFSET, 0.1)
